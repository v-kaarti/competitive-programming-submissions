...
void solve(int tc) {
    ...
    string ns = to_string(a);
    int pre = 0, S = ns.size(), diff;
    F0R(i, S) {
        if (d.contains(ns[i] - '0')) pre++;
        else {
            diff = ns[i] - '0';
            break;
        }
    ...
    R0F(i, pre + 1) {
        ...
        if (it != d.begin()) {
            --it;
            string s = ns.substr(0, i);
            s.push_back('0' + *it);
            s += string(S - i - 1, '0' + *d.rbegin());
            ans = min(ans, llabs(a - stoll(s)));
            break;
        }
    }
    ...
    if (S > 1) {
        string small(S - 1, '0' + *d.rbegin());
        ans = min(ans, llabs(a - stoll(small)));
    } else if (d.contains(0)) ans = min(ans, llabs(a));

    string big = "";
    auto it = d.begin();
    while (it != d.end() && *it == 0) it++;
    if (it != d.end()) {
        big.push_back('0' + *it);
        big += string(S, '0' + *d.begin());
        ans = min(ans, llabs(a - stoll(big)));
    }
