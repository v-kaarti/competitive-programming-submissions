...
void solve(int tc) {
    ...
    FOR(x, 1, n + 1) p.push_back(p.back() + v[x]);
    m.erase(prev(m.end()));

    int mn_a = 1, mx_a = n, mn_b = n + 1, mx_b = 0;
    auto adj_a = [&]() {
        while (mn_a <= n && above[mn_a] == 0) mn_a++;
        while (mx_a >= 0 && above[mx_a] == 0) mx_a--;
    };
    ...
    for (auto xs : m) {
        ...
        adj_a();
        int l = max(mn_a, mn_b), r = min(mx_a, mx_b); 
        if (l < r) ans += p[r - 1] - p[l - 1];
    }
    cout << ans << endl;
}
