...
void solve(int tc) {
    // figure out which positive to chump
    int n; cin >> n;
    vi a(n + 1);
    FOR(i, 1, n + 1) cin >> a[i];
    vi pfx(n + 1);
    FOR(i,1,n+1) pfx[i] = pfx[i - 1] + a[i];
    int absum = 0, bsum = accumulate(all(a), 0LL), chump = -1;
    FOR(i, 1, n + 1) {
        auto e = a[i];
        if (e <= 0) {
            absum += abs(a[i]);
