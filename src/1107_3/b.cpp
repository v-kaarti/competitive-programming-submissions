...
void solve(int tc) {
    ...
    auto dfs = [&](auto&& dfs, int a, int b, int curr, int ct) -> void {
        if (ans != -1 || ct > 10) return;
        if (curr >= 2 && ok(curr)) {
            ans = curr;
            return;
        }
        if (curr > 0 || a > 0) dfs(dfs, a, b, curr * 10 + a, ct + 1);
        if (curr > 0 || b > 0) dfs(dfs, a, b, curr * 10 + b, ct + 1);
    };
