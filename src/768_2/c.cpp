...
void solve(int tc){
    int n, k; cin >> n >> k;
    vpi pairs;
    F0R(i, n / 2) pairs.pb({i, n - 1 - i});
    int zloc = 0, mbit = __builtin_ctz(n) - 1;
    for (int p = 0; p <= mbit; ++p) {
        int check = 1LL << p;   
        if (k & (check)) {
            if (check == 1LL << mbit) swap(pairs[zloc].f, pairs[check - 1].s);
            else swap(pairs[zloc].f, pairs[check].f);
            zloc = check;
        }
    }
    int sum = 0;
    each(p, pairs) sum += p.f & p.s;
    if (sum != k) {
        cout << -1 << endl;
        return;
    }
    each(p, pairs) cout << p.f << " " << p.s << endl;
}
