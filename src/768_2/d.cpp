...
bool poss(int x, int y, int k, vi& a) {
    int in_range = upper_bound(all(a), y) - lower_bound(all(a), x);
    int out_range = a.size() - in_range;
    return in_range - out_range >= k;
}

int ok(int len, vi& a, int k) {
    each(e, a) {
        if (poss(e, e + len, k, a)) return e;
    }
    return -1;
}
...
void solve(int tc){
    ...
    int zer = 0, ones = 0, done = 0, prev = 0;
    F0R(i, n) {
        og[i] ? ones++ : zer++;
        if (done == k - 1) {
            cout << i + 1 << " " << n << endl;
