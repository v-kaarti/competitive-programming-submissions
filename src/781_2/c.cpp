...
int extra_time(const vi& a) {
    if (a.empty()) return 0;
    int l = -1, r = *max_element(all(a)) + 1;
    while (r - l > 1) {
        int m = l + (r - l) / 2, need = 0;
        each(x, a) need += max(0LL, x - m);
        if (need <= m) r = m;
        else l = m;
    }
    return r;
...
void solve(int tc){
    int n, p; cin >> n;
    V<vi> children(n + 1, vi());
    FOR(i, 2, n + 1) {
        cin >> p;
        children[p].pb(i);
    }
    vector<pair<int, int>> decaying;
    vector<int> big_groups;
    int small_groups = 1;
    int ts = 0;
    each(c, children) {
        if (c.size() == 1) small_groups++;
        if (c.size() > 1) big_groups.pb(c.size());
    }
    sort(all(big_groups), greater<int>());
    int m = big_groups.size(), elapsed = m + small_groups;
    FOR(i, 0, m) big_groups[i] = max(0LL, big_groups[i] - (m - i) - small_groups);
    elapsed += extra_time(big_groups);
    cout << elapsed << endl;
}
