...
void solve(int tc){
    int n; cin >> n;
    vi nodes(n); each(e, nodes) cin >> e;
    int i = 1, prev_count = 0, curr_count = 1, height = 0;
    while (i < n) {
        ppr(height, prev_count, curr_count);
        if (prev_count == 0) height++, prev_count = curr_count, curr_count = 0;
        curr_count++;
        while (i < n - 1 && nodes[i] < nodes[i + 1]) i++, curr_count++;
        i++;
        prev_count--;
    }
    cout << height << endl;
