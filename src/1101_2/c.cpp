...
void solve(int tc) {
    ...
    FOR(i, 0, n) {
        ...
        FOR(t, 0, x + 1) {
            if (dp[curr][t] == -1) continue;
            dp[next][t] = max(dp[next][t], dp[curr][t]);
            if (u[i] == 'I' || u[i] == 'A') {
                if (t + 1 <= x) dp[next][t + 1] = max(dp[next][t + 1], dp[curr][t] + 1);
            }
            if (u[i] == 'E' || u[i] == 'A') {
                int free = s * t - dp[curr][t];
                if (free > 0) {
                    dp[next][t] = max(dp[next][t], dp[curr][t] + 1);
                }
            }
        }
    }
    cout << *max_element(begin(dp[n % 2]), end(dp[n % 2])) << endl;
}
