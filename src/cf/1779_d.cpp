...
void solve(int tc){
    ...
    each(p, prio) {
        ...
        if (a[i] < b[i] || !razers.contains(e)) {
            ok = false;
            break;
        }
        // move right
        done.insert(i);
        razers.erase(razers.find(e));
        
        int ir = i;
        while (ir + 1 < n) {
            if (ranges.contains(ir)) {
                int start = ir, end = ranges[ir];
                ranges.erase(start);
                if (start != end) ranges.erase(end);
                ir = end;
                continue;
            }
            if (b[ir + 1] <= e) {
        ...
        int il = i;
        while (il - 1 >= 0) {
            if (ranges.contains(il)) {
                int end = il, start = ranges[il];
                if (end != start) ranges.erase(end);
                ranges.erase(start);
                il = start;
                continue;
            }
